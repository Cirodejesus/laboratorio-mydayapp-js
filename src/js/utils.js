import { todoList, nodeFooter, nodeMain, nodeTodoCount,
  completeButton, nodeFilters } from "./nodes";
...
const editingItem = (event) => {
  ...
  if (event.target.nodeName === "LABEL" && isNotNodeCompleted) {
    ...
    nodeInput.addEventListener('keydown', updateTask
    (nodeList, nodeFooter, taskId));
  }
}
...
export const selectFilter = () => {
  ...
  if (location.hash == '#/') {
    ...
    highlightFilter();
    const getLocalStorage = JSON.parse(localStorage.getItem("mydayapp-js")) || [];
    const pendingFilterStorage = JSON.parse(localStorage.getItem("mydayapp-js"))
    .filter(item => !item.completed) || [];
    ...
    highlightFilter();
    const getLocalStorage = JSON.parse(localStorage.getItem("mydayapp-js")) || [];
    const completeFilterStorage = JSON.parse(localStorage.getItem("mydayapp-js"))
    .filter(item => item.completed) || [];
